...
from pymc3.math import logdiffexp, log1mexp
from pymc3.theanof import floatX

__all__ = ["Bound", "Truncated"]
...
class _DiscreteTruncated(_Truncated, Discrete):
    def __init__(self, distribution, lower, upper, transform="infer", *args, **kwargs):
        if transform == "infer":
            transform = None
        if transform is not None:
...
class _ContinuousTruncated(_Truncated, Continuous):
    r"""
    An upper, lower or upper+lower bounded distribution
...
class Truncated:
    r"""
    Create a Truncated variable object that can be applied to create
    a new upper, lower, or upper and lower truncated distribution.

    The truncation bounds are inclusive for discrete distributions.
    ...
    def __call__(self, name, *args, **kwargs):
        ...
        transform = kwargs.pop("transform", "infer")
        if issubclass(self.distribution, Continuous):
            return _ContinuousTruncated(
                name, self.distribution, self.lower, self.upper, transform, *args, **kwargs
            )
        elif issubclass(self.distribution, Discrete):
            return _DiscreteTruncated(
                name, self.distribution, self.lower, self.upper, transform, *args, **kwargs
            )
        else:
            raise ValueError("Distribution is neither continuous nor discrete.")
    ...
    def dist(self, *args, **kwargs):
        if issubclass(self.distribution, Continuous):
            return _ContinuousTruncated.dist(
                self.distribution, self.lower, self.upper, *args, **kwargs
            )

        elif issubclass(self.distribution, Discrete):
            return _DiscreteTruncated.dist(
                self.distribution, self.lower, self.upper, *args, **kwargs
            )
        else:
            raise ValueError("Distribution is neither continuous nor discrete.")
